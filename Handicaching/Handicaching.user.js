...
// @match       *://www.geocaching.com/geocache/GC*
// @match       *://www.geocaching.com/seek/log.aspx*
...
// @icon        https://raw.githubusercontent.com/JRInge/userscripts/master/GeocacheQRCode/Handicaching/handicaching_icon.png
// @grant       GM_xmlhttpRequest
// @updateURL   http://geo.inge.org.uk/userscripts/handicaching.meta.js
// @downloadURL https://openuserjs.org/install/JRI/Handicaching.user.js
// ==/UserScript==
...
(function () {
  ...
  var gccode, target, t1;

  function getGCref() {
    var gcref,
      gcrefDiv = document.getElementById("ctl00_ContentBody_CoordInfoLinkControl1_uxCoordInfoCode"),
      gcrefDiv2 = document.getElementById("uxCoordInfoLink"),
      code = "";
    if (gcrefDiv) {
      code = gcrefDiv.innerHTML;
    } else {
      if (gcrefDiv2) {
        code = gcrefDiv2.innerHTML;
      }
    }
    gcref = code.match(/GC[A-Z0-9]+/);
    if (gcref) {
      return gcref[0];
    }
    console.warn("Handicaching couldn't work out GCcode");
    return null;
  }
  ...
  if (typeof GM_xmlhttpRequest !== "function") { // jshint ignore:line
    console.warn("Handicaching requires a web browser that supports GM_xmlhttpRequest");
    return;
  }
  console.info("Handicaching v0.0.6.");

  if (document.location.host === "www.handicaching.com") {
    // On handicaching.com
    target = document.getElementById("contenttitle");
    if (target) {
      target.innerHTML = target.innerHTML.replace(/GC[A-Za-z0-9]{1,5}/, function (code) { return "<a href='http://www.geocaching.com/geocache/" + code + "'>" + code + "</a>"; });
    }
  } else {
    // On geocaching.com
    if (document.getElementById("ctl00_ContentBody_LogBookPanel1_ddLogType")) {
      // Log editing page, no cache details available
      return;
    }
    ...
    if (t1) {
      ...
      t1 = document.getElementById("uxLog");
      if (t1) {
        target = t1;
      } else {
    ...
    if (target && gccode) {
      GM_xmlhttpRequest({ // jshint ignore:line
        ...
        onload: function (response) {
          var h, d, t,
            css = ".handistyle{border:1px solid #F98014;margin:1em 0 1em;padding:0.5em;-moz-border-radius: 0.5em;border-radius: 0.5em;clear:both;} .handistyle a{color:#F98014 !important;font-weight:bold;text-decoration:none;}",
            handiDiv = document.createElement("div"),
            handiStyle = document.createElement("style"),
          ...
          if (response.responseText.match(/Sorry, we could not find that cache in our database/)) {
            ...
            if (h && d) {
              rating += " is rated " + h[1] + " D" + d[1] + "/T" + t[1] + ". &nbsp;<a href='http://www.handicaching.com/show.php?waypoint=" + gccode + "'>" + goicon + " See details</a> &nbsp;";
            } else {
              console.warn("Handicaching userscript couldn't extract HCode from Handicaching.com");
              return;
            }
          }
          ...
          handiDiv.innerHTML = "<strong>Handicaching</strong> - Geocache accessibility ratings <a href='http://www.handicaching.com/' style='float:right;'>" + linkicon + "</a><br />" + rating + "<a href='http://www.handicaching.com/rate.php?waypoint=" + gccode + "&step=2'>" + goicon + " Rate this cache</a>";
          handiStyle.type = "text/css";
          if (handiStyle.styleSheet) {
            handiStyle.styleSheet.cssText = css;
          } else {
            handiStyle.appendChild(document.createTextNode(css));
          }
          document.documentElement.firstChild.appendChild(handiStyle);
          target.parentNode.insertBefore(handiDiv, target.nextSibling.nextSibling);
        }
      });
    } else {
      console.error("Handicaching: Unexpected document structure");
    }
  }
}());