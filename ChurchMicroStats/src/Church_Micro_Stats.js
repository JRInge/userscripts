  ...
  if (typeof GM_xmlhttpRequest === "function") { // jshint ignore:line
      ...
      onload: function (response) {
        ...
        if (response.responseText) {
          ...
          if (jsontext && jsontext.cmstats && jsontext.cmstats.length > 0) {
            displayStats(jsontext.cmstats, getCurrentPage());
          } else {
            if (jsontext.error) {
    ...
    handler.text = "function handleCMstats(jsonp) { " +
      ...
      "if (jsonp.cmstats && jsonp.cmstats.length > 0) { displayStats(jsonp.cmstats, '" + getCurrentPage() + "'); } else { console.error('Could not process Church Micro stats data.');}}";
